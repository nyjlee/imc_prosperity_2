...
class Trader:
    ...
    def starfruit_strategy(self, state: TradingState) -> List[Order]:
        ...
        if forecasted_price > best_bid + 2:
            ...
            orders.append(
                Order('STARFRUIT', math.floor(forecasted_price + spread / 2), int(math.floor(ask_volume / 2))))
    ...
    def coconut_strategy(self, state: TradingState) -> List[Order]:
        ...
        lags_val = self.mid_prices_history['COCONUT'][-20]
        ...
        current_volatility = std / sma if sma != 0 else 0
        print(f'The current volatility is: {current_volatility}')
        ...
        if current_volatility > volatility_threshold:
            ...
            if coconut_mid_price < lower_band:
                coconut_orders.append(Order('COCONUT', self.get_best_ask('COCONUT', state)[0], buy_volume_coconut))
            elif coconut_mid_price > upper_band:
                coconut_orders.append(Order('COCONUT', self.get_best_bid('COCONUT', state)[0], sell_volume_coconut))
        else:

            if coconut_mid_price > lags_val:
    ...
    def coconut_strategy210(self, state: TradingState) -> List[Order]:
        """
        Combined mean reversion and trend following strategy using moving averages and Bollinger Bands.
        """
        coconut_orders = []
        ...
        bollinger_std = 2  # Standard deviations for Bollinger Bands

        # Historical Prices
        prices = self.mid_prices_history['COCONUT'][-long_window:]
        short_sma = statistics.mean(prices[-short_window:])
        long_sma = statistics.mean(prices)
        std = statistics.stdev(prices)
        current_price = self.get_mid_price('COCONUT', state)

        # Calculate Bollinger Bands
        upper_band = short_sma + bollinger_std * std
        lower_band = short_sma - bollinger_std * std

        # Trend following signals
        if short_sma > long_sma:
            # If short SMA is above long SMA, consider a bullish trend
            if current_price > short_sma and current_price < upper_band:
                coconut_orders.append(Order('COCONUT', self.get_best_ask('COCONUT', state)[0], buy_volume_coconut))
            elif current_price < short_sma and current_price > lower_band:
                coconut_orders.append(Order('COCONUT', self.get_best_bid('COCONUT', state)[0], sell_volume_coconut))
        else:
            # If short SMA is below long SMA, consider a bearish trend
            if current_price < short_sma and current_price > lower_band:
                coconut_orders.append(Order('COCONUT', self.get_best_bid('COCONUT', state)[0], sell_volume_coconut))
            elif current_price > short_sma and current_price < upper_band:
                coconut_orders.append(Order('COCONUT', self.get_best_ask('COCONUT', state)[0], buy_volume_coconut))

        return coconut_orders, coconut_coupon_orders

    def coconut_strategy2(self, state: TradingState) -> List[Order]:
        """
        Buying and Selling based on last trade price vs mean price (ceiling floor version)
        """
        coconut_orders = []
        ...
        short_window = 20
        long_window = 50
        short_ma = sum(lags[-short_window:]) / short_window
        long_ma = sum(lags[-long_window:]) / long_window

        if short_ma > long_ma:
            # Check if coconut_mid_price is above the short MA
            if coconut_mid_price > short_ma:
                coconut_orders.append(Order('COCONUT', coconut_ask, buy_volume_coconut))
        # Trend Confirmation: If short MA crosses below long MA, consider it a downtrend
        elif short_ma < long_ma:
            # Check if coconut_mid_price is below the short MA
            if coconut_mid_price < short_ma:
                coconut_orders.append(Order('COCONUT', coconut_bid, sell_volume_coconut))
    ...
    def coconut_strategy3(self, state: TradingState) -> List[Order]:
        """
        Buying and Selling based on last trade price vs mean price (ceiling floor version)
        """
        coconut_orders = []
        ...
        # Trend Confirmation: If short MA crosses below long MA, consider it a downtrend
            ...
            # MACD Indicator
            short_ema = sum(lags[-12:]) / 12
            long_ema = sum(lags[-26:]) / 26
            macd_line = short_ema - long_ema
            signal_line = sum(lags[-9:]) / 9

            if macd_line > signal_line and coconut_mid_price > short_ma:
                coconut_orders.append(Order('COCONUT', self.get_best_ask('COCONUT', state)[0], buy_volume_coconut))
            elif macd_line < signal_line and coconut_mid_price < short_ma:
                coconut_orders.append(Order('COCONUT', self.get_best_bid('COCONUT', state)[0], sell_volume_coconut))

        return coconut_orders, coconut_coupon_orders

    def coconut_strategy220(self, state: TradingState) -> List[Order]:
        """
    ...
    def coconut_strategy230(self, state: TradingState) -> List[Order]:
        ...
        # Historical Prices
        highs = self.high_prices_history['COCONUT'][-atr_window:]
        lows = self.low_prices_history['COCONUT'][-atr_window:]
        closes = self.close_prices_history['COCONUT'][-atr_window:]


        true_ranges = [max(h - l, abs(h - c_prev), abs(l - c_prev)) for h, l, c_prev in zip(highs, lows, closes[:-1])]
        ...
        take_profit = current_price + atr_multiplier * atr
    ...
    def nicolas_cafe(self, state: TradingState) -> List[Order]:
        """
        Buying and Selling based on last trade price vs mean price (ceiling floor version)
        """
        coconut_orders = []
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        try:
            result['COCONUT'], result['COCONUT_COUPON'] = self.nicolas_cafe(state)
