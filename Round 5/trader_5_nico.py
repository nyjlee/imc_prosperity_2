...
class Trader:
    ...
    observation_history = {"ORCHIDS": [], "SUNLIGHT": [], "HUMIDITY": []}
    observation_diff_history = {"ORCHIDS": [], "SUNLIGHT": [], "HUMIDITY": []}
    ...
    def __init__(self) -> None:
        ...
        self.max_iv =  0.1697578300721943
        self.min_iv = 0.14901861264929178
    ...
    def update_diff_orchids(self, diff_observations, observation_history):

        for symbol in ['ORCHIDS','SUNLIGHT']:
            while len(observation_history[symbol]) > 50:
                observation_history[symbol].pop(0)
            
            if len(observation_history[symbol]) == 50:
                resampled = observation_history[symbol][::10]
                diff = resampled[-1] - resampled[-2]
                diff_observations[symbol].append(diff)

            while len(diff_observations[symbol]) > 8:
                diff_observations[symbol].pop(0)
    ...
    def orchids_strategy3(self, state : TradingState) -> List[Order]:
        ...
        self.observation_history['SUNLIGHT'].append(sunlight)
        self.observation_history['HUMIDITY'].append(humidity)
        self.observation_history['ORCHIDS'].append(mid_price)

        if len(self.observation_diff_history['SUNLIGHT']) >= 3 and len(self.observation_diff_history['HUMIDITY']) >= 3:
            L1_orchids = self.observation_diff_history['ORCHIDS'][-1]
            L1_sunlight = self.observation_diff_history['SUNLIGHT'][-1]
            L1_humidity = self.observation_diff_history['HUMIDITY'][-1]

            L2_orchids = self.observation_diff_history['ORCHIDS'][-2]
            L2_sunlight = self.observation_diff_history['SUNLIGHT'][-2]
            L2_humidity = self.observation_diff_history['HUMIDITY'][-2]

            L3_orchids = self.observation_diff_history['ORCHIDS'][-3]
            L3_sunlight = self.observation_diff_history['SUNLIGHT'][-3]
            L3_humidity = self.observation_diff_history['HUMIDITY'][-3]

            #L4_orchids = self.observation_diff_history['ORCHIDS'][-4]
            #L4_sunlight = self.observation_diff_history['SUNLIGHT'][-4]
            #L4_humidity = self.observation_diff_history['HUMIDITY'][-4]

            #L5_orchids = self.observation_diff_history['ORCHIDS'][-5]
            #L5_sunlight = self.observation_diff_history['SUNLIGHT'][-5]
            #L5_humidity = self.observation_diff_history['HUMIDITY'][-5]
        else:
            return orders, conversion
        
        estimated_diff =  -0.051261 + L1_orchids * -0.007738 + L1_sunlight * 0.003845
        + L1_humidity * -0.016205 + L2_orchids * -0.011018 + L2_sunlight * 0.006492
        + L2_humidity * -0.233520 + L3_orchids * 0.014333 + L3_sunlight * -0.012204
        + L3_humidity * 1.099917 

        estimated_price =  mid_price + estimated_diff

        print('estimated price:',estimated_price)

        print('est diff', estimated_diff)

        """
        intercept = 693.3028
        beta_sunlight = 0.0401
        beta_humidity = 3.7793

        estimated_price = intercept + beta_sunlight * sunlight + beta_humidity * humidity
        """

        if estimated_price > best_ask:
            orders.append(Order('ORCHIDS', math.floor(best_ask), bid_volume))

        if estimated_price < best_bid:
            orders.append(Order('ORCHIDS', math.floor(best_bid), ask_volume )) 
    ...
    def orchids_strategy4(self, state : TradingState) -> List[Order]:
        ...
        if len(self.observation_history['HUMIDITY']) > 250:
            humidity_diff = max(self.observation_history['HUMIDITY']) - min(self.observation_history['HUMIDITY'])
            print('humidity diff', humidity_diff)
        else:
            return orders, conversion
    ...
    def coconut_strategy(self, state : TradingState) -> List[Order]:
        ...
        if implied_vol > self.max_iv:
            self.max_iv = implied_vol
        if implied_vol < self.min_iv:
            self.min_iv = implied_vol

        iv_rank = (implied_vol - self.min_iv) / (self.max_iv - self.min_iv) * 100
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        self.update_diff_orchids(self.observation_diff_history, self.observation_history)
        ...
        for product in self.pnl_tracker.keys():
            ...
            while len(self.observation_history['HUMIDITY']) > 260:
                self.observation_history['HUMIDITY'].pop(0)
        ...
        try:
            result['ORCHIDS'] = self.orchids_strategy4(state)[0]
        except Exception as e:
        ...
        conversions = self.orchids_strategy4(state)[1]
        #conversions = 0
