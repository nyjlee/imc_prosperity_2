...
class Trader:
    ...
    def amethysts_strategy2(self, state: TradingState) -> List[Order]:
        ...
        if position_amethysts == 0:
            ...
            # Short position
            orders.append(
                Order('AMETHYSTS', math.floor(self.ema_prices['AMETHYSTS'] - 1), int(math.ceil((bid_volume / 2)))))
            orders.append(
                Order('AMETHYSTS', math.floor(self.ema_prices['AMETHYSTS'] - 2), int(math.floor((bid_volume / 2)))))
            orders.append(Order('AMETHYSTS', math.ceil(self.ema_prices['AMETHYSTS'] + 3), ask_volume))
    ...
    def starfruit_strategy3(self, state: TradingState) -> List[Order]:
        ...
        lags = self.prices_history['STARFRUIT'][-20:]
        a = 0
        for i in lags:
            a += i
        ma = a / len(lags)
        ...
        sd = self.calculate_standard_deviation(lags)
        print("The sd is ", sd)
        upper_limit = ma + 1.5 * sd
        lower_limit = ma - 1.5 * sd
    ...
    def starfruit_strategy4(self, state: TradingState) -> List[Order]:
        ...
        lags = self.prices_history['STARFRUIT'][-2]

        if last_price < lags:
            orders.append(Order('STARFRUIT', best_ask, ask_volume))
            #orders.append(Order('STARFRUIT', math.ceil(mid_price + 1), ask_volume))
        elif last_price > lags:
            orders.append(Order('STARFRUIT', best_bid, bid_volume))
