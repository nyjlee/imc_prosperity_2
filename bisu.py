...
class Trader:
    ...
    def amethysts_strategy2(self, state: TradingState) -> List[Order]:
        ...
        if position_amethysts == 0:
            ...
            orders.append(
                Order('AMETHYSTS', math.ceil(self.ema_prices['AMETHYSTS'] + 1), int(math.ceil((ask_volume / 2)))))
            orders.append(
                Order('AMETHYSTS', math.ceil(self.ema_prices['AMETHYSTS'] + 2), int(math.floor((ask_volume / 2)))))

        elif position_amethysts < -15:
            # Short position
            orders.append(
                Order('AMETHYSTS', math.floor(self.ema_prices['AMETHYSTS'] - 2), int(math.ceil((bid_volume / 2)))))
            orders.append(
                Order('AMETHYSTS', math.floor(self.ema_prices['AMETHYSTS'] - 1), int(math.floor((bid_volume / 2)))))
    ...
    def coconut_strategy(self, state: TradingState) -> List[Order]:
        ...
        implied_vol = self.implied_volatility(coconut_coupon_mid_price, coconut_mid_price, 10000, 246 / 250,
                                              initial_sigma=0.2,
